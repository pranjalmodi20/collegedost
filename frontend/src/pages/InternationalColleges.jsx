...
const InternationalColleges = () => {
  ...
  const [country, setCountry] = useState(searchParams.get('country') || 'USA');
  ...
  const handleCountryChange = (c) => {
      setCountry(c);
      setPage(1);
      setSearchTerm(''); // Optional: clear search on country switch
  };
            ...
            <div className="flex gap-4 mt-8 border-b border-gray-100 pb-1 overflow-x-auto">
                {['USA', 'UK', 'Canada', 'Australia'].map((c) => (
                    <button
                        ...
                        className={`px-6 py-2 font-medium text-sm transition-all whitespace-nowrap relative ${
                            country === c 
                            ...
                            {colleges.map((college, index) => (
                                <tr key={college._id} className="hover:bg-blue-50/50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        ...
                                        {/* <div className="text-xs text-gray-500 bg-gray-100 inline-block px-2 py-0.5 rounded mt-1">{college.type || 'Public'}</div> */}
                                    </td>
