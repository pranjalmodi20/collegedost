...
const TestPrepDetailPage = () => {
    const { stream, exam, type } = useParams();
    const [data, setData] = useState([]);
    const [loading, setLoading] = useState(true);

    // Helper map to fix case sensitivity for API query
    const examMap = {
        'jee-main': 'JEE Main',
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                // Construct Query API
                const queryExam = examMap[exam] || exam;
                const queryType = typeMap[type] || type;
                const queryStream = stream.charAt(0).toUpperCase() + stream.slice(1); // Engineering

                const res = await api.get(`/test-prep?stream=${queryStream}&exam=${encodeURIComponent(queryExam)}&type=${queryType}`);
                if (res.data.success) {
...
export default TestPrepDetailPage;
