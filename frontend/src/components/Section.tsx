...
import { FaStar, FaMapMarkerAlt, FaLocationArrow, FaArrowRight, FaDownload, FaPaperPlane, FaUniversity } from 'react-icons/fa';
...
const Section: React.FC<SectionProps> = ({ title, items, type = 'card', viewAllLink }) => {
    return (
        <section className="py-16 relative">
            ...
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between mb-10">
                    ...
                    <Link href={viewAllLink || (title.includes('Colleges') ? '/colleges' : '#')} className="flex items-center gap-2 text-sm font-bold text-brand-indigo hover:text-brand-orange transition-colors group">
                                ...
                                <motion.div whileHover={{ y: -10 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="h-full">
                                    <div className="bg-white rounded-2xl overflow-hidden h-full flex flex-col shadow-md hover:shadow-2xl transition-shadow duration-300 group border border-gray-100 relative">
                                        <div className="p-5 flex gap-4 items-start relative">
                                            ...
                                            <div className="w-16 h-16 rounded-xl p-1 bg-white shadow-sm border border-gray-100 flex-shrink-0 relative z-10 flex items-center justify-center overflow-hidden">
                                                {item.logo ? <img src={item.logo} alt={item.name || item.title || ''} className="w-full h-full object-contain rounded-lg" /> : <FaUniversity className='text-gray-300 text-2xl' />}
                                            </div>
                                            <div className="relative z-10 flex-1 min-w-0">
                                                <h3 className="text-base font-bold mb-2 text-gray-900 line-clamp-2 leading-tight group-hover:text-brand-indigo transition-colors font-heading">
                                                    <Link href={item.link || item.href || '#'} className="before:absolute before:inset-0">{item.name || item.title}</Link>
                                                </h3>
...
const CategoryContent = ({ item }: { item: SectionItem }) => {
    return (
        <>
